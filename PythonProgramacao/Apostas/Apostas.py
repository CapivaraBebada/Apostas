...
class Funcao():
    ...
    def Conecta_db(self):
        self.Conecta = sqlite3.connect('Apostas.bd')
        self.Cursor = self.Conecta.cursor()
    def Desconecta_bd(self):
        self.Conecta.close()
   
    # Aqui, o que for digitado nas entradas será conectado com o preenchimento das colunas
    def Montar_Tabela(self):
        self.Conecta_db()
        # Preenchendo tabela
        self.Cursor.execute("""
                            CREATE TABLE IF NOT EXISTS Apostas (
                                cod INTEGRER PRIMARY KEY,
                                Competicao CHAR(40) NOT NULL
                            );
                            """)
        # Para validar as informações doBanco de Dados
        self.Conecta.commit()
        # Para saber se está sendo concetado ao banco:
...
class Application(Funcao):
    ...
    # Botão GANHOU
        self.Botao_ganhou = Button(self.Frame_1, bg='#323437', font=('Amble', 15, 'bold'), text='GANHOU', activebackground='green', foreground='white')
    ...
    def Tabela_Apostas(self):
        # Criando colunas
        self.tabela = ttk.Treeview(self.Frame_1, height=50, columns=('col_1', 'col_2', 'col_3', 'col_4', 'col_5', 'col_6', 'col_7', 'col_8'))
        self.tabela.heading('#0', text='Data')
        self.tabela.heading('#1', text='Competição')
        self.tabela.heading('#2', text='Casa')
        self.tabela.heading('#3', text='Fora')
        self.tabela.heading('#4', text='Mercado')
        self.tabela.heading('#5', text='Odd')
        self.tabela.heading('#6', text='Unidade')
        self.tabela.heading('#7', text='L/P')
        self.tabela.heading('#8', text='Montante')
        # Definindo tamanho de colunas
        self.tabela.column('#0', width=8)
        self.tabela.column('#1', width=50)
        self.tabela.column('#2', width=40)
        self.tabela.column('#3', width=40)
        self.tabela.column('#4', width=40)
        self.tabela.column('#5', width=1)
        self.tabela.column('#6', width=1)
        self.tabela.column('#7', width=1)
        self.tabela.column('#8', width=1)
        # Definindo lugar
    ...
    def Tabela_Valores(self):
        ...
        self.tabela_valores.heading('#1', text='Valor')
